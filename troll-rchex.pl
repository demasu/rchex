...
my $accesshash_file    = '/root/.accesshash';
...
my $ruby_status = ($ruby_found_path eq $ruby_req_path) ? sub { status_message("ok", "Ruby binary looks good."); ( my $ruby_version_status = ($ruby_version eq $ruby_req_target) ? status_message("ok", "Ruby version matches target version: $ruby_version") : status_message("fatal", "Ruby version not at target: $ruby_version installed, $ruby_req_target expected") ) } : status_message("warning", "Installed Ruby may not be the one cPanel installes; keep an eye out.");
